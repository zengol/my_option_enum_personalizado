...
enum MightHaveAValue<'a> {
 ...
 fn main() {
    ...
    let audiobook = Media::AudioBook { 
     ...
     /* let item = catalog.get_by_index(4333);
     println!("{:#?}", item); */

    /*  En términos prácticos, esto es lo que sucede:

    catalog.get_by_index(50) devuelve un MightHaveAValue.
    El match intenta "abrir" este MightHaveAValue.
    Si resulta ser un ThereIsAValue, entonces:
        La "caja" se abre.
        Lo que está dentro (una referencia a Media) se asigna a la variable value.
        Ahora puedes usar value para acceder a ese Media.
    Si no es un ThereIsAValue, este patrón no coincide y se pasa al siguiente caso. */
     match catalog.get_by_index(50) {
        //MightHaveAValue::ThereIsAValue es como una caja que puede contener algo 
        //(en este caso, una referencia a Media).
